...
while True:
    ...
    print("##### accXe = %fG  ##### accYe =   %fG  ##### accZe =  %fG  #####" % ( xACCe, yACCe, zG))

    

    alpha = 0.5
    accIgTh = 0.03             #acc ignore thresh
    ...
    if (count == 1):
        ...
        if (abs(xG-LPx)>accIgTh):
           LPx = (LPx*(1-alpha)) + (xG * alpha)
        if (abs(yG-LPy)>accIgTh):
            LPy = (LPy*(1-alpha)) + (yG * alpha)
        if (abs(zG-LPz)>accIgTh):
            LPz = (LPz*(1-alpha)) + (zG * alpha)
    xG = LPx
    yG = LPy
    zG = LPz
